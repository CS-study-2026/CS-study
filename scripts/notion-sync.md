...
- 파일명은 Notion 페이지의 `제목`을 기준으로 만들고, 파일 시스템에서 쓸 수 없는 문자만 공백으로 바꿉니다.
- 제목이 `9-4. ...`처럼 한 자리 번호로 시작하면 GitHub 정렬을 위해 파일명만 `09-4. ...`처럼 보정합니다.
- Markdown 상단 정보 표에는 `title` 다음에 페이지 작성자 이름을 `author`로 넣습니다.
